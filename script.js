...
function num() {
  return Math.ceil(data.length / tpage);
}
...
function change(page) {
  ...
  //for (let i = (1 - 1) * 10 = 0; i < (1 * 10) && i < 100; i++)

  for (let i = (page - 1) * tpage; i < (page * tpage) && i < data.length; i++) {
      listing_table.innerHTML +=
          `
      <tr>
  ...
  btn_next.style.display = (page === num()) ? 'none' : 'inline-block';
}
...
for (let i = 0; i < bt.length; i++) {
  bt[i].addEventListener("click", (e) => {
      e.preventDefault();
      change(parseInt(e.target.innerText))
  });
}
